...
class DeepLab(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet101', pretrained=True, 
...
class DeepLab_smx(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet101', pretrained=False, 
                output_stride=16, freeze_bn=False, **_):
         
            #上面把backbone从xception改到了resnet101
        super(DeepLab_smx, self).__init__()
        assert ('xception' or 'resnet' in backbone)
        if 'resnet' in backbone:
            self.backbone = ResNet(in_channels=in_channels, output_stride=output_stride, pretrained=pretrained)
            low_level_channels = 256
