...
class dicelossmulticlass():
    ...
    def __call__(self, probs: Tensor, target: Tensor,eps=1e-7) -> Tensor:
        ...
        # print('num_classes, probs.shape', num_classes, probs.shape)
        if num_classes == 1:
            ...
            # print("target.squeeze(1).shape",target.squeeze(1).shape)
            true_1_hot = torch.eye(num_classes).to('cuda')[target.squeeze(1).to(torch.int64)]
            # true_1_hot = torch.eye(num_classes)[target.squeeze(1).to(torch.int64).to(probs.device)]
            # print("true_1_hot",true_1_hot)
            true_1_hot = true_1_hot.permute(0, 3, 1, 2).float()
...
#         print(pc.shape, tc.shape)

        loss = 0
        for i in range(0,probs.shape[1]):
            pc1 = pc[:,i].unsqueeze(1)
            tc1 = tc[:,i].unsqueeze(1)
            intersection: Tensor = einsum("bcwh,bcwh->bc", pc1, tc1)
            union: Tensor = (einsum("bcwh->bc", pc1) + einsum("bcwh->bc", tc1))
...
class DiceLoss3d():
    ...
    def __call__(self, probs: Tensor, target: Tensor) -> Tensor:
        #assert simplex(probs) and simplex(target)
        tmp_device = target.get_device()
#         print("probs: ",probs.shape, "target: ",target.shape,'target max: ',int((target).max()))
        ...
        if target.shape[1] ==1 and (int((target).max()) > 1):
            target2 = torch.ones(size=(probs.shape),device=torch.device(tmp_device))
            
            temp_target = target[:,0] + 1
            temp_target[temp_target!=1]  = 0
            target2[:,0]= temp_target
            
            temp_target = target[:,0] 
            temp_target[temp_target!=1]  = 0
            target2[:,1]= temp_target
            
            temp_target = target[:,0] -1
            temp_target[temp_target!=1]  = 0
            target2[:,2]= temp_target

#         print("probs: ",probs.shape, "target: ",target2.shape)
        
        
        pc = probs[:, ...].type(torch.float32)
        tc = target2[:, ...].type(torch.float32)
...
#         print(output.shape, target.shape)
        ...
        dice_loss = (self.dice(output, target))/2
        return CE_loss + dice_loss
    
class CE_DiceLoss_nchannel(nn.Module):
    # def __init__(self, smooth=1, reduction='mean', ignore_index=255, weight=torch.tensor([0.5,2,2,2,2]).cuda()):
    def __init__(self, smooth=1, reduction='mean', ignore_index=255, weight=None ):
        super(CE_DiceLoss_nchannel, self).__init__()
        self.smooth = smooth
#         self.dice = DiceLoss()
#         self.cross_entropy = nn.CrossEntropyLoss(weight=weight, reduction=reduction, ignore_index=ignore_index)
        self.cross_entropy =  CrossEntropyLoss2d_cedice()
...
class CrossEntropyLoss2d_cedice(nn.Module):
    def __init__(self, weight=None, size_average=True, ignore_index=255):
    # def __init__(self, weight=torch.tensor([1.,2.,4.]).cuda(), size_average=True, ignore_index=255):
    # def __init__(self, weight=torch.tensor([0.5,1]).cuda(), size_average=True, ignore_index=255):   
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2, alpha=0, ignore_index=255, size_average=True):
        super(FocalLoss, self).__init__()
        self.gamma = gamma
        self.size_average = size_average
        # self.CE_loss = nn.CrossEntropyLoss(reduce=False, ignore_index=ignore_index,weight=torch.tensor([1,1]).cuda() )
        self.CE_loss = nn.CrossEntropyLoss(reduce=False, ignore_index=ignore_index,weight=None )
...
class ce_loss(nn.Module):
    ...
    def forward(self, output, target, weights = None, ignore=255):
...
class tverskylossmulticlass():
    def __init__(self, **kwargs):
        # Self.idc is used to filter out some classes of the target mask. Use fancy indexing
        #self.idc: List[int] = kwargs["idc"]
        print(f"Initialized")

    def __call__(self, output: Tensor, target: Tensor, alpha = 0.3, beta = 0.7, eps=1e-7) -> Tensor:
...
class IOUmulticlass():
    ...
    def __call__(self, output: Tensor, target: Tensor, eps=1e-7) -> Tensor:
...
class TD():
    ...
    def __call__(self, probs: Tensor, target: Tensor,eps=1e-7, alpha = 0.3, beta = 0.7) -> Tensor:
        #assert simplex(probs) and simplex(target)
        num_classes = probs.shape[1]
#         print(num_classes, probs.shape)
        if num_classes == 1:
...
#             print(target.squeeze(1).shape)
        ...
        denom = intersection + (alpha * fps) + (beta * fns)
        tversky_loss = (num / (denom + eps)).mean()
        return (1 - dice_loss) + (1 - tversky_loss)
