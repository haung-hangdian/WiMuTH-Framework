...
class cbam(nn.Module):
    ...
    def __init__(self, img_ch=3, n_classes=2, init_features=32, network_depth=5, reduction_ratio=None, n_skip=4, n_head = 3, att_mode = 'cbam', is_head_selective = False, is_shortcut = False, conv_type = 'basic', activation = nn.ReLU()):
#     def __init__(self, img_ch=3, n_classes=2, init_features=32, network_depth=5, reduction_ratio=None, n_skip=4, n_head = 3, att_mode = 'cbam', is_head_selective = False, is_shortcut = False, conv_type = 'basic', activation = nn.ReLU()):     
...
#         return seg_output, cls_output
        return seg_output
        return self.softmax(seg_output)  # 在最后的输出前用softmax函数
class sk(nn.Module):
    '''reduction_ratio = None -> U-NET
    '''

    def __init__(self, img_ch=3, n_classes=2, init_features=32, network_depth=5, reduction_ratio=8, n_skip=4, att_mode = None, is_shortcut = False, conv_type = 'sk', activation = nn.ReLU()):
#nn.LogSoftmax(dim=1)
#     def __init__(self, img_ch=3, n_classes=2, init_features=32, network_depth=5, reduction_ratio=8, n_skip=4, att_mode = None, is_shortcut = False, conv_type = 'sk', activation = nn.ReLU()):
    ...
    def forward(self, x):
        ...
        # 添加softmax层
        seg_output = self.softmax(seg_output)  
        smx = torch.nn.Softmax2d()
        seg_output = smx(seg_output)

#         return seg_output, cls_output
        return seg_output

class skold(nn.Module):
    '''reduction_ratio = None -> U-NET
    '''
