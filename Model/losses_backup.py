...
#         print(inputs.shape,tmp_targets.shape)
...
#         self.gamma = gamma
#         self.size_average = size_average
#         self.CE_loss = nn.CrossEntropyLoss(reduce=False, ignore_index=ignore_index,weight=torch.tensor([1,300]).float().cuda() )
# #         self.CE_loss = nn.CrossEntropyLoss(reduce=False, ignore_index=ignore_index,weight=torch.tensor(0.25).float().cuda() )
# #         self.CE_loss = nn.CrossEntropyLoss2d(reduce=False, ignore_index=ignore_index,weight=alpha )
...
class FocalLoss(nn.Module):
    def __init__(self, weight=torch.tensor([1,30]).float().cuda() , size_average=True, ignore_index=255):
        super(FocalLoss, self).__init__()
        self.nll_loss = nn.NLLLoss(weight, size_average, ignore_index)
